...
exports.createMessage = function (req, res, next) {
    ...
    let newMessage = new Messages.model(message);
    // console.log("new", newMessage)

    newMessage.save(function (err, messageData) {
        if (err) {
            throw err;
        }
        // console.log(messageData);
        User.findOneAndUpdate({ _id: message.senderId }, {
            $push: {
                chats: messageData
            }
        }, (err, user) => {
            if (err) {
                ...
                console.log('bsbsbs', user)
                res.send(user);
            }
        });
        User.findOneAndUpdate({ _id: message.receiverId }, {
            $push: {
                chats: messageData
            }
        }).catch(err => {
...
exports.getMessages = function (req, res, next) {
    User.findOne({ _id: req.body.id }, (err, updatedUser) => {
        console.log("upd", updatedUser.chats);
        // res.status(201).json({
