...
// const socket = io.connect('http://seemeapp.herokuapp.com/')
...
class MatchList extends React.Component {
  ...
  componentWillMount() {
    ...
    if (user !== undefined) {
      let url = 'http://seemeapp.herokuapp.com/api/matches';
      ...
      // socket.on('connect', () => {
      //   let url = 'http://seemeapp.herokuapp.com/api/update-socket';
      //     axios.put(url,
  ...
  renderList() {
    ...
    } else if (user.matches.length === 0 && window.location.href !== 'http://seemeapp.herokuapp.com/swatch') {
      return (
      ...
      return (
        <div className="match-list-container">
          {this.state.matches.map((item) => {
            // console.log(item);
            let urlId = 'http://seemeapp.herokuapp.com/see/' + item._id;
            return (
              <div className="match">
